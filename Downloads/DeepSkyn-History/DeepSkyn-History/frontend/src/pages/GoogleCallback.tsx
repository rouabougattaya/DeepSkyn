  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        ...
        setSession({
          accessToken: backendData.accessToken, // Matches backend SessionTokens
          refreshToken: backendData.refreshToken || 'google_refresh_fallback',
          accessTokenExpiresAt: backendData.accessTokenExpiresAt || new Date(Date.now() + 3600000).toISOString(),
          refreshTokenExpiresAt: backendData.refreshTokenExpiresAt || new Date(Date.now() + 7 * 24 * 3600000).toISOString(),
          user: {
            ...backendData.user,
            authMethod: 'google'
          }
