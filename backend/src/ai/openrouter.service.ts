            ...
            try {
                response = await axios.post(
                    `${this.baseUrl}/chat/completions`,
                    {
                        ...
                        max_tokens: 2000 // Limite pour éviter l'erreur 402/crédits insuffisants
                    },
                    {
                        headers: {
...
`;
    ...
    async generateSVRRoutine(products: any[], profile: any): Promise<any> {
        ...
        const productList = products.map(p =>
            `- [${p.category.toUpperCase()}] "${p.name}" | Texture: ${p.texture || 'N/A'} | Score: ${p.score ?? 'N/A'}/10 | Ingredients: ${p.ingredients.join(', ')} | Price: €${p.price} | URL: ${p.url} | Description: ${p.description} | Suits: ${p.suitableSkinTypes.join(', ')} | Concerns: ${p.suitableConcerns.join(', ')}`
        ).join('\n');
...
`;
        ...
        try {
            ...
            this.logger.log(`✅ SVR routine: ${parsed.recommendedProducts?.length} products recommended, ${parsed.morning.length} AM + ${parsed.night.length} PM steps`);
            return parsed;
