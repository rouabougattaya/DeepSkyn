...
export default function SubscriptionCard() {
  ...
  return (
    <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-6 overflow-hidden relative group">
      <div className={`absolute top-0 right-0 w-32 h-32 opacity-10 blur-2xl rounded-full -mr-10 -mt-10 ${plan === 'PRO' ? 'bg-teal-500' : plan === 'PREMIUM' ? 'bg-purple-500' : 'bg-slate-500'}`}></div>
      
      <div className="flex justify-between items-start mb-4 relative z-10">
        <div>
          <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-1">Current Plan</h3>
          <div className="flex items-center gap-2">
            <PlanIcon size={24} className={plan === 'PRO' ? 'text-teal-600' : plan === 'PREMIUM' ? 'text-purple-600' : 'text-slate-600'} />
        ...
        <div className={`p-4 rounded-xl border flex items-center justify-between ${planStyles[plan]}`}>
          <div>
            ...
            <div className="text-sm font-bold">
              {plan === 'FREE' ? 'Basic limits applied' : plan === 'PRO' ? 'Expanded access' : 'Unlimited capabilities'}
            </div>
          </div>
          <button
