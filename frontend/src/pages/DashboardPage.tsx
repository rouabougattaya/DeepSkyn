...
import {
  Brain, Camera, BarChart3, Shield, History, Sparkles, RefreshCw
} from 'lucide-react';
...
export default function ProfessionalDashboard() {
  ...
  const loadSkinAgeRecommendations = async (insightData?: SkinAgeInsightResponse | null) => {
    ...
    if (insight?.products && insight.products.length > 0) { setRecommendedProducts(insight.products); return; }
    const searchCandidates = [
      ...(insight?.productSuggestions?.filter(Boolean) || []),
      ...(insight?.status === 'older' ? ['anti-aging', 'anti aging', 'retinol'] : ['hydration', 'moisturizer', 'spf']),
    ...
    try {
      for (const term of searchCandidates) {
        if (!term) continue;
        for (const attempt of [{ search: term, isClean: true }, { search: term, isClean: undefined }]) {
          const products = await productService.filter({ ...attempt, sortBy: 'rating', sortOrder: 'DESC' });
          const top = (products || []).slice(0, 6);
          if (top.length > 0) { setRecommendedProducts(top); return; }
        }
      }
      const anyTop = await productService.filter({ sortBy: 'rating', sortOrder: 'DESC' });
      setRecommendedProducts((anyTop || []).slice(0, 6));
  ...
  const handleLaunchAnalysis = async () => { navigate('/ai-demo'); };
      ...
      <main style={{ maxWidth: '1280px', margin: '0 auto', padding: '32px 24px 56px', marginTop: '12px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 40, padding: '20px 24px', borderRadius: 20, background: 'linear-gradient(120deg, #f0fdfa, #e0f2fe)', border: `1px solid ${THEME.border}`, boxShadow: '0 14px 28px rgba(13,148,136,0.12)' }} className="kpi-card">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={handleRefreshAI} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', borderRadius: 12, background: THEME.surface, border: `1px solid ${THEME.border}`, fontWeight: 600, fontSize: 14, color: THEME.textPrimary, cursor: 'pointer' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 32 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
            ...
            {/* SKIN AGE INSIGHT */}
            {isPro ? (
              <SkinAgeInsightCard insight={skinAgeInsight} loading={skinAgeLoading} productsLoading={productsLoading}
                onRetry={loadSkinAgeInsights} onRefreshProducts={() => loadSkinAgeRecommendations(skinAgeInsight)}
                onLaunchAnalysis={handleLaunchAnalysis} recommendedProducts={recommendedProducts} />
            ) : (
              <div style={{ position: 'relative', minHeight: 200 }}>
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(10px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 10, borderRadius: 20 }}>
                  ...
                  <p style={{ color: THEME.textSecondary, fontSize: 16, marginBottom: 24, maxWidth: 300, textAlign: 'center' }}>Unlock detailed skin age analysis and personalized recommendations</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
                  {[
                    ...
                    { label: 'Lowest Score', value: metrics?.worstScore ?? 0, sub: 'Min across scores', gradient: 'linear-gradient(135deg, #f43f5e, #e11d48)', icon: <TrendingDown size={22} /> },
                ...
                <div style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)', border: `1px solid ${THEME.border}`, borderRadius: 20, padding: 24, display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 20 }}>
                  {[
                    ...
                    { label: 'Q1 (25%ile)', value: (metrics?.percentile25 ?? 0).toFixed(1), sub: 'Bottom 25%', color: '#ef4444', border: true },
                ...
                <div style={{ background: '#ffffff', border: `1px solid ${THEME.border}`, borderRadius: 16, padding: '18px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 40, height: 40, borderRadius: 12, background: metrics?.trendDirection === 'up' ? '#f0fdf4' : metrics?.trendDirection === 'down' ? '#fef2f2' : '#f8fafc', display: 'grid', placeItems: 'center' }}>
                  ...
                  <div style={{ padding: '8px 20px', borderRadius: 24, background: metrics?.trendDirection === 'up' ? 'linear-gradient(135deg, #10b981, #059669)' : metrics?.trendDirection === 'down' ? 'linear-gradient(135deg, #f43f5e, #e11d48)' : 'linear-gradient(135deg, #94a3b8, #64748b)', color: 'white', fontSize: 13, fontWeight: 800 }}>
                ...
                {trends.length > 0 && (
                  <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(trends.length, 4)}, 1fr)`, gap: 14 }}>
                    {trends.map(t => {
                      ...
                      return (
                        <div key={t.period} style={{ background: isUp ? 'linear-gradient(180deg, #f0fdf4, #fff)' : isDown ? 'linear-gradient(180deg, #fef2f2, #fff)' : 'linear-gradient(180deg, #f8fafc, #fff)', border: `1px solid ${THEME.border}`, borderRadius: 18, padding: 20 }}>
                ...
                <div style={{ background: '#ffffff', border: `1px solid ${THEME.border}`, borderRadius: 24, padding: 28, boxShadow: '0 4px 24px rgba(0,0,0,0.06)', position: 'relative', overflow: 'hidden' }}>
                  <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: 'linear-gradient(90deg, #0d9488, #10b981, #8b5cf6, #f43f5e)' }} />
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: 11, color: THEME.textSecondary, marginTop: 2 }}>Historical skin health • Last 6 months</p>
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 16, fontSize: 11, color: '#64748b', fontWeight: 600 }}>
                      {[{ color: '#0d9488', label: 'Average' }, { color: '#8b5cf6', label: 'Best' }, { color: '#f43f5e', label: 'Lowest' }].map(l => (
                  ...
                  {monthly.filter(m => m.analysisCount > 0).length > 0 ? (
                      ...
                      <Line
                        data={{
                          ...
                          datasets: [
                            ...
                            { label: 'Lowest score', data: monthly.filter(m => m.analysisCount > 0).map(m => m.worstScore), borderColor: '#f43f5e', backgroundColor: 'transparent', fill: false, tension: 0.4, pointBackgroundColor: '#f43f5e', pointBorderColor: '#ffffff', pointBorderWidth: 2, pointRadius: 3, pointHoverRadius: 6, borderWidth: 1.5, borderDash: [3, 3] },
                          ],
                        }}
                        options={{
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 24px', background: 'linear-gradient(180deg, #f8fafc 0%, #ffffff 100%)', borderRadius: 16, border: '2px dashed #e2e8f0' }}>
                      ...
                      <Link to="/ai-demo" style={{ marginTop: 20, padding: '10px 24px', borderRadius: 12, background: 'linear-gradient(135deg, #0d9488, #10b981)', color: 'white', fontWeight: 700, fontSize: 13, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8, boxShadow: '0 4px 12px rgba(13,148,136,0.3)' }}>
            ...
            <div>
              ...
              <Link to="/ai-demo" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '14px 24px', background: `linear-gradient(135deg, ${THEME.primary}, #0f766e)`, borderRadius: 14, boxShadow: `0 4px 12px ${THEME.primary}40`, color: 'white' }} className="kpi-card">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  <Link key={action.to} to={action.to} className="group bg-white p-6 rounded-3xl border border-slate-200 hover:border-[#0d9488]/30 hover:shadow-xl hover:shadow-teal-500/5 transition-all">
              ...
              <p className="text-xs text-slate-400 mt-4 ml-2">ℹ️ 4 actions available • Manage your profile and analyses</p>
            </div>
