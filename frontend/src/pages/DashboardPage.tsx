...
export default function ProfessionalDashboard() {
  ...
  const loadSkinMetrics = async () => {
    ...
    try {
      const [m, t, mon, hist, fullHist] = await Promise.all([
        dashboardService.getMetrics(),
        dashboardService.getTrends(),
        dashboardService.getMonthlyData(6),
        comparisonService.getUserAnalyses(1, 4),
        comparisonService.getUserAnalyses(1, 15), // Fetch more for timeline
      ]);
      setMetrics(m);
      setTrends(t);
      setMonthly(mon);
      ...
      const formattedTimeline = fullHist.data.map((a: any) => ({
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: THEME.background, fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <aside style={{ width: '240px', background: THEME.surface, borderRight: `1px solid ${THEME.border}`, position: 'fixed', top: '64px', height: 'calc(100vh - 64px)', zIndex: 100, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '24px 16px' }}>
          ...
          <SidebarItem icon={Smartphone} label="Sessions" onClick={() => navigate('/sessions')} />
        ...
        <div style={{ marginTop: 'auto', padding: '24px', borderTop: `1px solid ${THEME.border}` }}>
          ...
          <button onClick={handleLogout} style={{ width: '100%', padding: '10px', borderRadius: 12, border: `1px solid ${THEME.border}`, background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, cursor: 'pointer', fontSize: 13, fontWeight: 600, color: '#ef4444' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: 11, color: THEME.textSecondary, marginTop: 2 }}>
                        Évolution détaillée de votre score cutané sur les dernières analyses
                      </p>
                    </div>
                  </div>
                  <Link to="/analysis/history" style={{ fontSize: 12, fontWeight: 700, color: THEME.primary, textDecoration: 'none' }}>
                    Détail complet
                  </Link>
                </div>

                <div style={{ height: 300 }}>
                  <TimelineView data={timelineData} height="100%" showTitle={false} />
                </div>
              </div>
            </div>
