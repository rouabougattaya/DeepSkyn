...
export default function LoginFingerprintPage() {
  ...
  const handleFingerprintLogin = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(options.message || "Biometric error")
        return
      }
      ...
      if (!verifyRes.ok) {
        setError(data.message || "Fingerprint not recognized")
        return
      }
      ...
      if (err.name === "NotAllowedError") {
        setError("Authentication cancelled or denied.")
      } else {
    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center p-4">
      ...
      <div className="w-full max-w-[440px] mt-20">
        ...
        <div className="space-y-6">
          ...
          >
            <Fingerprint size={18} />
            {isLoading ? "Signing in..." : "Sign in with fingerprint"}
          </Button>
