...
export default function RegisterFingerprintPage() {
  ...
  const registerFingerprint = async () => {
    ...
    try {
      ...
      if (!available) {
        setError("Aucun authentificateur biométrique disponible.")
        return
      }

      // 1️⃣ Demander options backend
      const optionsRes = await fetch(
        `${API_URL}/auth/register-fingerprint/options`,
        {
          method: "POST",
      ...
      if (!optionsRes.ok) {
        setError(options.message || "Erreur génération biométrique")
        return
      }
      ...
      if (!credential) {
        setError("Échec création credential")
        return
      }
  ...
  response: {
    ...
    clientDataJSON: bufferToBase64url(response.clientDataJSON),
      ...
      const verifyRes = await fetch(
        `${API_URL}/auth/register-fingerprint/verify`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (!verifyRes.ok) {
        setError(data.message || "Erreur vérification")
        return
      }
      ...
      <div className="w-full max-w-[440px] mt-12">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 text-sm">
            Inscription avec empreinte digitale
          </p>
        </div>
        ...
        <div className="space-y-5">
          ...
          >
            <Fingerprint size={18} />
            {isLoading ? "Vérification..." : "Enregistrer empreinte digitale"}
          </Button>
        </div>
